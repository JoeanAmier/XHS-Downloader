...
def get_ui_messages() -> dict[str, str]:
    ...
    return {
        ...
        "settings.file_delay": _("单个文件下载间隔"),
        "settings.file_delay_help": _("每个文件下载完成后等待的时长，用于降低风控风险"),
        "settings.note_interval": _("单个作品处理间隔"),
        "settings.note_interval_help": _("每个作品处理完成后等待的时长，用于降低风控风险"),
