...
(function () {
    let novice = GM_getValue("novice", true);
    GM_registerMenuCommand(`新手引导模式 ${novice ? '✔️' : '❌'}`, function (command) {
        GM_setValue("novice", !novice);
        novice = GM_getValue("novice", true);
        alert('修改 XHS-Downloader 设置成功！');
    });
    ...
    function scrollScreen(callback) {
    ...
    function extractWorkInfo(index) {
        const notesRawValue = unsafeWindow.__INITIAL_STATE__.user.notes._rawValue[index];
        return new Set(notesRawValue.map(({id}) => id));
    }

    function generateUrls(index) {
        return [...extractWorkInfo(index)].map(id => `https://www.xiaohongshu.com/explore/${id}`).join(" ");
    }

    function confirmBox() {
        return confirm("即将开始自动提取账号作品链接\n提取完毕会自动将作品链接复制到剪贴板\n如果您未登录小红书账号，可能无法获取账号全部作品链接\n此提示可在 Tampermonkey 菜单关闭\n是否立即开始提取？");
    }

    function getAllPostLinks(callback, index) {
        if (!novice || confirmBox()) {
            scrollScreen(() => {
                let urlsString = generateUrls(index);
                callback(urlsString);
            })
        }
    }

    function getAllPostLinksEvent(index = 0) {
        getAllPostLinks(urlsString => {
            navigator.clipboard.writeText(urlsString).then(() => {
                alert('作品链接已复制到剪贴板！\n搭配 XHS-Downloader 程序可以实现批量下载作品！');
            }).catch(error => {
    ...
    function updateContainer() {
        ...
        // 按钮列表
        let buttons = [createButton('xhsDownloadButton', '下载无水印作品文件', window.location.href.includes("https://www.xiaohongshu.com/explore/"), deal), createButton('xhsGetLinksButton', '提取账号发布作品链接', window.location.href.includes("https://www.xiaohongshu.com/user/profile/"), getAllPostLinksEvent)];
